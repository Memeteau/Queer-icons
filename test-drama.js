...
function play(s,category='Courage'){
 if(!s.locked&&!s.ended)act(s,s.leader,'choose',{category});
}
...
test('Gala: neuf scores, quatre cartes réellement engagées',()=>{
 ...
 act(s,0,'power');assert(s.locked);assert.equal(s.dq.reveal[0].value,1440);assert.equal(s.dq.reveal[1].value,1080);assert.deepEqual(s.hands.map(h=>h.length),[12,4]);unique(s);
});
...
test('Polémique reprend la main sans avancer la boule disco',()=>{
 const s=state(['plain','guy-hocquenghem']);s.leader=0;act(s,1,'power');assert.equal(s.leader,1);assert.equal(s.dq.pending,null);assert.equal(s.dq.turn,0);assert.equal(s.dq.charges[1],2);assert.throws(()=>act(s,1,'power'),/Tu as la main/);
});
...
test('Plan random remplace toutes les cartes avant le choix',()=>{
 const s=state(['maurice-sachs','plain']),old=s.hands.map(h=>h[0].id);act(s,0,'power');assert.equal(s.dq.pending,null);s.hands.forEach((h,i)=>assert.notEqual(h[0].id,old[i]));play(s,'Miracle');assert(s.locked);unique(s);
});
...
test('Gala résout immédiatement le pli sans validation supplémentaire',()=>{
 const s=state(['natalie-clifford-barney','plain'],5);act(s,0,'power');assert(s.locked);assert.equal(s.dq.turn,1);assert.equal(s.dq.pending,'gala');assert.equal(s.dq.reveal.length,2);unique(s);
});
...
test('Le leader ne peut activer aucun des 21 atouts avant son choix',()=>{
 for(const id of Object.keys(D.POWERS)){
  const s=state([id,'plain']);s.leader=0;
  const before=JSON.stringify(s);assert.match(D.canPower(s,0),/Tu as la main/);
  assert.throws(()=>act(s,0,'power',{target:1}),/Tu as la main/);assert.equal(JSON.stringify(s),before);
 }
});
test('Un non-leader active son atout avant le choix ; le choix résout immédiatement',()=>{
 const s=state(['henri-iii','plain']);act(s,0,'power');assert.equal(s.dq.charges[0],2);
 assert.equal(D.scores(s,0,s.hands[0][0]).Courage,35);act(s,1,'choose',{category:'Courage'});assert(s.locked);assert.equal(s.dq.turn,1);
 assert.throws(()=>act(s,0,'power'),/Action indisponible/);
});
test('100 parties 2–6 joueurs: cartes conservées et scores de base intacts',()=>{
 ...
 for(let seed=1;seed<=100;seed++){
  ...
  for(let k=0;k<100&&!s.ended;k++){
   ...
   if(!s.ended&&!s.locked)act(s,s.leader,'choose',{category:D.allowed(s)[0]},rng);
