...
(() => {
  ...
  let localSeat=0,selectedUltimate='Courage',error='',powerOpen=false,lastReactionKey='';
  const WIN_LINES=['Slay !','Show them, bitch !','Yaaaaaas Queen !','Pense à t’hydrater pendant que tu gagnes tous les plis…','These gurls should have learned their herstory !'];
  const LOSE_LINES=['Girl please…','Not born ready','Tu retouchais ton make up pendant le jeu ?','Ne montre pas tes larmes'];
  ...
  const style=document.createElement('style');style.textContent=`
    ...
    .dramaCharges{display:flex;gap:5px;justify-content:flex-end;align-items:center}.dramaCharge,.dramaChargeButton{font-size:31px;color:#ffe36c;text-shadow:0 0 9px #ffd54a,0 0 18px #ffbd2d;line-height:1}.dramaCharge.off{color:#59435f;text-shadow:none;opacity:.35}.dramaChargeButton{appearance:none;border:0;background:transparent;padding:0 1px;cursor:pointer;filter:none}.dramaChargeButton:focus-visible{outline:2px solid #fff;outline-offset:3px;border-radius:4px}
    .powerBox{margin-top:10px;padding:12px;border:1px solid #9a70b2;border-radius:16px;background:#24132d}.powerBox h3{color:#ffe58a}.powerBox button{margin-top:8px}.dramaError{padding:10px;border:1px solid #ff94ba;border-radius:10px;color:#ffbed3}.dramaPanel select{margin-bottom:10px}.dramaPanel button:focus-visible,.dramaPanel select:focus-visible{outline:3px solid #fff;outline-offset:3px}
    ...
    @keyframes discoPulse{50%{transform:rotate(9deg) scale(1.13)}}@keyframes goldShimmer{to{filter:brightness(1.25);text-shadow:0 0 8px #fff8c9,0 0 22px #ffd84c,0 0 38px #f0a900}}@keyframes purpleSmoke{to{filter:brightness(1.12)}}@keyframes smokeDrift{to{transform:translateX(8px) scale(1.05);opacity:.7}}@keyframes reclaimZoom{0%{transform:scale(.15);opacity:0}55%{transform:scale(1.18);opacity:1}100%{transform:scale(1);opacity:1}}@keyframes sparkPop{from{transform:scale(.4) rotate(-10deg);opacity:.35}to{transform:scale(1.3) rotate(12deg);opacity:1}}
    @media(max-width:520px){.dramaPanel{padding:18px 14px}.dramaLine{grid-template-columns:70px minmax(0,1fr) auto;gap:10px}.discoBall{font-size:50px}.dramaCharge,.dramaChargeButton{font-size:27px}}
    @media(prefers-reduced-motion:reduce){.discoWarm,.dqWin,.dqLose,.dqLose:before,.dqLose:after,.dqReclaimText,.dqSpark{animation:none}}
  ...
  function dispatch(action,payload={}){error='';powerOpen=false;if(context().active){window.QI_MULTIPLAYER.dramaAction?.(action,{...payload,revision:S.dq.revision});return;}try{D.applyAction(S,action==='next'?S.leader:actor(),action,{...payload,revision:S.dq.revision});if(action==='next')localSeat=S.leader;}catch(e){error=e.message;}renderDrama();}
  function cardView(i){const revealed=S.locked&&S.dq.reveal.find(x=>x.i===i);return revealed?.card||S.hands[i]?.[0];}
  function ensureReactionSlots(){const card=cardPanel();if(!card)return{};let win=card.querySelector('.dqWin');if(!win){win=document.createElement('div');win.className='dqWin hidden';card.insertBefore(win,card.querySelector('.portrait'));}let lose=card.querySelector('.dqLose');if(!lose){lose=document.createElement('div');lose.className='dqLose hidden';const head=card.querySelector('.cardhead');head.after(lose);}return{win,lose};}
  ...
  function renderDrama(){
    ...
    $('stats').innerHTML='';
    if(c&&!S.ended){const values=S.locked?c.scores:D.scores(S,i,c,true),allowed=D.allowed(S);for(const meta of CATEGORY_META){const row=document.createElement('div');row.className='stat';for(const [k,v]of Object.entries({cat:meta.color,'cat-soft':meta.soft,'cat-faint':meta.faint,'cat-dark':meta.dark}))row.style.setProperty('--'+k,v);row.innerHTML=`<button class="statbutton" type="button"><span class="catIcon"><img src="${escape(meta.icon)}" alt=""></span><span class="statText"><span class="statName">${escape(meta.name)}</span><span class="statComment">${escape(c.comments?.[meta.name]||'')}</span></span></button><span class="score">${values[meta.name]}</span>`;const b=row.querySelector('button');b.disabled=S.locked||!!d.pending||i!==S.leader||!allowed.includes(meta.name);b.onclick=()=>dispatch('choose',{category:meta.name});$('stats').append(row);}}
    renderReactions(i,d);
    ...
    const stars=Array.from({length:3},(_,k)=>{
      if(k>=inf.charges)return '<span class="dramaCharge off">★</span>';
      if(canShowPower)return '<button type="button" class="dramaChargeButton" aria-label="Voir l’atout de cette carte" title="Atout">★</button>';
      return '<span class="dramaCharge">★</span>';
    }).join('');
    panel.innerHTML=`<div class="dramaLine"><span aria-hidden="true" class="discoBall ${opened?'discoReady':warming?'discoWarm':''}">🪩</span><div><h3 class="dramaTitle">Drama Queen</h3><span class="dramaPhase" role="status">${phase}</span></div><div class="dramaCharges" aria-label="${inf.charges} atouts sur 3">${stars}</div></div>
      ${canShowPower&&powerOpen?`<div class="powerBox"><h3>${escape(inf.power.name)}</h3><p>${escape(inf.power.text)}</p>${inf.power.target?`<label for="dramaTarget">Adversaire</label><select id="dramaTarget">${S.hands.map((h,j)=>h.length&&j!==i?`<option value="${j}">${escape(n[j])}</option>`:'').join('')}</select>`:''}<button id="dramaPower" class="secondary" ${D.canPower(S,i)?'disabled':''}>Activer · 1 atout</button>${D.canPower(S,i)?`<small>${escape(D.canPower(S,i))}</small>`:''}</div>`:''}
    ...
    panel.querySelectorAll('.dramaChargeButton').forEach(btn=>btn.onclick=()=>{powerOpen=!powerOpen;renderDrama();});
    ...
    $('table').innerHTML='';
    if(S.locked&&d.reveal.length){for(const x of d.reveal){const el=document.createElement('div');el.className='seat'+(d.last?.winner===x.i?' winner':'');el.innerHTML=`<div class="row"><strong>${escape(n[x.i])} · ${escape(x.card.name)}</strong><strong>${x.value}</strong></div><small>${x.cardCount} carte(s) engagée(s)</small>`;$('table').append(el);}}else $('table').innerHTML=S.hands.map((h,j)=>`<div class="seat">${escape(n[j])} · ${h.length} carte(s)${j===S.leader?' · LEADER':''}</div>`).join('');
    $('result').classList.toggle('hidden',!S.locked&&!S.ended);if(S.locked||S.ended){const msg=d.last?.message||'';$('result').innerHTML=`<strong>${escape(msg)}</strong>${d.ultimate?'<br>'+d.ultimate.totals.map(x=>`${escape(n[x.i])} : ${x.value} points`).join('<br>'):''}`;}
    $('nextBtn').classList.toggle('hidden',!S.locked||S.ended||context().active&&!context().isHost);$('nextBtn').textContent='Manche suivante';
  }
