...
(() => {
  ...
  function renderDrama(){
    ...
    panel.innerHTML=`<div class="dramaLine"><span aria-hidden="true" class="discoBall ${opened?'discoReady':warming?'discoWarm':''}">🪩</span><div><h3>Drama Queen</h3><span role="status">${phase}</span></div><strong>${inf.charges}/3 atouts</strong></div>
      ...
      ${!S.ended&&!S.locked?`<p>${pending?`Catégorie retenue : <strong>${escape(pending)}</strong>. Chacun joue son atout ou passe.`:i===S.leader?'Tu as la main : choisis une catégorie ou active ton atout.':`${escape(n[S.leader])} choisit la catégorie. Tu peux déjà activer ton atout.`}</p>
